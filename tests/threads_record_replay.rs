...
fn build_fixture() -> PathBuf {
    ...
    let status = Command::new("make")
        ...
        .status()
        .unwrap();
    assert!(status.success(), "failed to build threads fixture");
    manifest_dir.join("test/threads")
}

/// The fixture's thread interleaving depends on timing when recording, so replay has to follow
/// the recorded thread switches to reproduce it.
#[test]
fn record_and_replay_threads_match() {
    let fixture = build_fixture();
    let test_dir = unique_test_dir();
    std::fs::create_dir_all(&test_dir).unwrap();
    let trace_path = test_dir.join("threads.trace");
    ...
    assert!(record_stdout.contains("order: "), "{record_stdout}");
    assert!(record_stdout.contains("sum: 155"), "{record_stdout}");
